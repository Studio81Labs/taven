...
Claim vybírá sealed verzi skutečně doručeného jobu/fáze jako důkaz výsledku a cestu ke kanonické geometrii i zákaznické konfiguraci, ne jako production snapshot pro další stroj. Po smazání zdrojového STEP nebo 3MF tak stále reprodukuje správný obsah, zatímco každý reroutovaný reprint odvodí nové machine-specific vstupy až z čerstvé eligibility a rezervace. Před doručením se `reproduction_delete_after` nenastaví; po doručení nesmí být dříve než příslušné `claim_until`. Aktivní incident zásilky, reklamace nebo právní hold termín prodlouží. Po odpadnutí poslední překážky mazací job odstraní artefakt i jeho produkční verze a ponechá jen auditní metadata a hash, nikoli rekonstruovatelnou geometrii.
...
| `ProductionReservation` | ✓ | ✓ | atomická skupina inventory + capacity rezervací se snapshoty candidate profilu/kalibrace/configu |
...
| `ReplacementFulfilmentAuthorization` | ✓ | ✓ | jednorázové oprávnění k handoff náhrady z claimu bez znovuotevření terminální objednávky |
| `AuditEvent` | — | ✓ | |
...
lost → recovered
```
`lost` a `returned` automaticky otevřou `Claim` proti zásilce, i když objednávka ještě není `delivered`. Volba `reship_pending` má tvrdý guard `status ∈ {returned, recovered} && custody_confirmed_at != null && re_qc_passed_at != null`; teprve fyzicky převzatý a znovu zkontrolovaný výrobek smí dostat nový `Shipment` s `replaces_shipment_id` a Claim skončí `resolved_reship` až jeho doručením. Dokud zásilka zůstává `lost`, povolené volby jsou jen rezervovaný `reprint_pending` nebo `refund_pending` — prázdný reshipment nikdy nevznikne.
...
**Claim**
```
opened → investigating → resolved_rejected | reship_pending | reprint_pending | refund_pending
...
Pro post-delivery claim otevřený do snapshotovaného `claim_until` čte přechod do `reprint_pending` kanonickou geometrii a přijatou `PrintConfigRevision` z rodičovského `ReproductionArtifact`; sealed verze skutečně doručeného jobu/fáze zůstává důkaz a `reproduces_artifact_version_id`. Její `CandidateResourceEstimate`, `MachineProfile`, `MachineCalibration` ani production `SliceResult` se do reprintu nekopírují. Čerstvá eligibility a `ProductionReservation` mohou vybrat jiný stroj a acceptance náhradního Jobu z nich vytvoří a později sealne novou machine-specific verzi. Flow tak nezávisí na smazaném zdrojovém `ModelFile`, ale ani nefalšuje vstupy nového stroje. Před doručením nemá reprodukční artefakt expiraci; otevřený shipment incident nebo claim prodlouží jeho retenci až do terminálního vyřešení.
...
`failed` ukládá `failure_stage = preparation | gcode | machine | printing | post_print | post_qc | packing` a důvod. Větve před tiskem pokrývají odstoupení makera/G-code/poruchu; `printing` zmetek; `printed | photo_submitted` ztrátu nebo poškození při dokončení; `qc_approved | packed` poškození při balení nebo ztrátu před předáním dopravci. `photo_submitted → qc_rejected` zůstává další terminální neúspěšná větev. Všechny pre-handoff neúspěchy používají stejnou auditovanou `ReplacementRequest`/cancellation recovery; až `handed_over` převádí ztrátu na `Shipment` incident. Post-QC failure vždy přepne dotčenou fázi z `qc_passed` do `recovery_pending`; u finální/jediné fáze totéž přepne agregát z `qc_passed | awaiting_balance | ready_to_ship`, zneplatní aktivní `balance_due_at` a zachová existující `PaymentSchedule` i capture. Po QC náhrady se fáze i případný agregát vrátí do `qc_passed`: při `amount_due = 0` agregát pokračuje do `ready_to_ship`, jinak do `awaiting_balance` s novým `balance_due_at`; nemůže tedy nechat objednávku v odesílatelném stavu bez výrobku ani požadovat již uhrazený doplatek podruhé. Měří se odděleně a nezhoršuje tiskový FPY.

Handoff je příkaz `handoff_shipment`, protože opakovatelnou fyzickou jednotkou je parcela, ne Job. Každý Job smí plnit více `FulfilmentSlot`, ale všechny musejí patřit jedinému `ShipmentPlan`; jeden plán naopak může čekat na více Jobů. Příkaz v jedné transakci ověří, že všechny aktuální Job lineage leaf přiřazených slotů jsou `packed`, přepne právě jeden Shipment do `handed_over` a všechny jeho přispívající Joby `packed → handed_over`. U běžné finální nebo jediné fáze navíc vyžaduje `amount_due = 0`, `refundable_balance = 0` a buď `Order.status = ready_to_ship` pro první parcelu, nebo `Order.status = shipped` pro další dosud nepředaný plán téže QC-passed fáze; první handoff posune agregát do `shipped`, další jej tam ponechají. U mezifázového sample vyžaduje `OrderPhase.status = qc_passed` a `phase_amount_due = 0` podle aktuálního `PaymentSchedule`. Claimová náhrada vyžaduje QC náhradního jobu a dosud nepoužité `ReplacementFulfilmentAuthorization` pro přesně tento Job a Claim a v téže transakci autorizaci spotřebuje. Původní Order si zachová `in_production | shipped` při pre-delivery incidentu nebo svůj terminální stav po doručení. Žádný Job proto nemůže být předán dvakrát ani blokovat handoff další parcely.
...
Pokud novou rezervaci nelze získat, `ReplacementRequest` zůstane auditovatelně `pending_capacity`, job se nevytvoří ani nenabídne a systém opakuje hledání jen do provozního deadline. Pak se request z výrobního selhání i neúspěšná fáze zruší: objednávka bez jediného doručeného slotu přejde do `cancelled → refunded`, objednávka s alespoň jedním doručeným slotem se po vrácení nedoručené části uzavře `partially_fulfilled`. Request vytvořený Claimem místo změny původního Order přepne Claim do `refund_pending` fallbacku popsaného výše. Potvrzená objednávka tedy musí mít aktivní/úspěšný listový job, otevřený `ReplacementRequest` s deadlinem, nebo finanční vypořádání.
...
12. Revidovaný `ModelFile` nesmí aktivovat batch bez nového deterministického slice, cenového snapshotu, čerstvého `EligibilitySnapshot`, atomicky získané nové `ProductionReservation`, přijetí `OrderRevision` zákazníkem a `revision_amount_due = 0`; požadavek na materiál a kapacitu platí i při zlevnění, odmítnutý batch končí finančně vypořádaným `partially_fulfilled` agregátem.
...
16. Závazná cena a capture platby vyžadují neprázdný, čerstvý `EligibilitySnapshot`; alespoň jeden způsobilý uzel musí mít pro vybranou barvu `available_g ≥ required_material_g` i nekolidující strojové intervaly a capture smí začít až po atomickém vytvoření společné `ProductionReservation` pro celou právě aktivovanou fázi. Počáteční capture fázované objednávky aktivuje a rezervuje jen sample; batch se znovu vyhodnotí a rezervuje až při vlastní aktivaci.
...
20. `reship_pending` vyžaduje fyzicky převzatou zásilku `returned | recovered` a nové QC; `resolved_reship` vznikne až doručením navázané zásilky. `lost` bez custody smí zvolit jen `reprint_pending` nebo `refund_pending`.
21. Selhání `Job` po QC a před handoff musí převést dotčenou fázi do `recovery_pending`; u finální/jediné fáze přejde do recovery i agregát. Návrat do odesílatelného stavu vyžaduje QC náhrady a znovuvyhodnocení `amount_due` bez druhého capture již uhrazené částky.
22. `handoff_shipment` smí v jedné transakci předat právě jednu parcelu jen tehdy, když všechny její sloty mají `packed` aktuální Job leaf a každý přispívající Job patří výhradně jejímu `ShipmentPlan`; běžná finální/jediná fáze navíc vyžaduje nulové finanční zůstatky a `Order.ready_to_ship` pro první parcelu nebo `Order.shipped` pro další plán, mezifázový sample `OrderPhase.qc_passed` a `phase_amount_due = 0`, claimový reprint jednorázové oprávnění přesně pro daný Claim a Job.
...
24. Úspěšný capture smí potvrdit objednávku nebo revizi jen s dosud neexpirovanou `ProductionReservation`; opožděný webhook musí zdroje atomicky znovu získat, a pokud to nelze, nesmí vytvořit Job a musí zachycenou platbu okamžitě kompenzovat refundem.
25. Před potvrzením fitu se fyzické zdroje rezervují jen pro sample; cenově zamčený batch získá vlastní čerstvou `ProductionReservation` až při aktivaci, takže přeprava ani recovery vzorku nemůže držet batch zdroje bez deadline.
...
`ProductionReservation` má `payment_reservation_expires_at = created_at + payment_reservation_minutes` po dobu nedokončené platby nebo nepřijaté revize: neúspěch či timeout ji atomicky uvolní. Webhook úspěšného capture ji smí přepnout na `held` jen pokud v téže transakci stále platí a vlastní oba zdroje. Opožděný capture po expiraci se idempotentně pokusí získat čerstvý candidate estimate a novou společnou rezervaci; při neúspěchu objednávku/revizi nepotvrdí, nevytvoří Job a zachycenou platbu okamžitě přepne do `refund_pending`. `Held` po platném capture nemá krátké checkout TTL, ale podléhá provoznímu deadline aktivní fáze. U fázované objednávky počáteční checkout rezervuje pouze sample; cenově zamčený batch získá vlastní zdroje až atomicky při potvrzení fitu nebo přijetí revize. Přijetí jobu přepne gramáž na `allocated` a intervaly na `scheduled`, nikoli ještě na spotřebu; až `printing` commitne materiál k průběžnému vyúčtování. Pre-print failure uvolní celou gramáž a nevyužitou kapacitu, in-print failure zapíše skutečnou spotřebu a vrátí zbytek, storno obdobně uvolní všechny nepoužité zdroje. V v0 je množina jediný vlastní stroj a paleta obsah jeho tři AMS; s heterogenní sítí se nikdy nesmí nabízet barva ze stroje, který ostatní požadavky, celou gramáž a plánované intervaly zakázky nesplní.
