...
Příkaz `handoff_job` má společný transakční guard s přechodem zásilky do `handed_over`. U finální nebo jediné fáze vyžaduje `Order.status = ready_to_ship`, `amount_due = 0` a `refundable_balance = 0`; u mezifázového sample vyžaduje `OrderPhase.status = qc_passed` a `phase_amount_due = 0` podle aktuálního `PaymentSchedule`. Pouhá možnost kliknout na Job `packed → handed_over` tedy nemůže obejít finanční ani fulfilment stav agregátu.
...
22. `handoff_job` smí v jedné transakci předat zásilku jen z finančně autorizované fáze: finální/jediná fáze vyžaduje `Order.ready_to_ship`, `amount_due = 0` a `refundable_balance = 0`; mezifázový sample vyžaduje `OrderPhase.qc_passed` a `phase_amount_due = 0`.
