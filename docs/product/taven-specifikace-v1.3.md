...
| `LateCaptureCompensation` | ✓ | ✓ | `balance_settlement` nebo `sample_confirmation_expired`; plná refundace provider capture doručeného až po cutoffu, bez znovuotevření fulfilmentu |
...
Refund incident-backed Claimu proti **nedoručenému sample** je prerequisite abort celé dosud neaktivované dávky, ne izolovaný refund jedné parcely. Stejný příkaz proto zamkne všechny fáze a sloty Orderu i jejich cenové alokace a rozšíří `claim_credit_scope` o všechny batch sloty a veškeré jejich dosud nekreditované component allocations. Batch atomicky přepne z `locked | awaiting_revision | awaiting_capacity` do `cancelled`, uzavře případné capture window revize postupem u confirmation timeoutu níže a uvolní krátkou rezervaci; `PriceAdjustment(kind = claim_slot_credit)` spotřebuje součet `remaining_contract_value` reklamovaných sample slotů i celého zrušeného batch scope. Po úspěchu všech příslušných refund webhooků jsou reklamované sample sloty a všechny batch sloty `cancelled_refunded`, batch fáze `cancelled_refunded` a Claim `resolved_refund`. Není-li doručený žádný jiný slot, Order projde `cancelled → refunded`; zůstává-li některý sample slot doručený, skončí až po stejné finanční bariéře `partially_fulfilled`. Zamčená předplacená dávka tak nikdy nezůstane bez výrobní cesty ani bez creditu.
