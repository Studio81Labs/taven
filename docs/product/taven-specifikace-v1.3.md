...
cancellation_pending → handed_over (jen přes úplný context-specific handoff po doloženém scanu před voidem)
...
```
single: active → in_production → qc_passed → shipped → delivered → completed
        active | in_production | qc_passed → cancelled
...
41. Claim refund musí před platebním refundem aktivovat idempotentní `claim_slot_credit` o součet `remaining_contract_value` celého zamčeného `claim_credit_scope` reklamovaných slotů po předchozích component credits; u prerequisite abortu nedoručeného sample se batch odebere samostatným `batch_cancellation_credit` a tatáž alokace se nesmí z ceny odečíst ani refundovat dvakrát.
...
62. Refund child remedy ve `reship_pending | reprint_pending | replacement_in_production` smí začít až po dokončení `ClaimRemedyCancellation`: všechny jeho pre-handoff Shipment leaf musí být provider-confirmed `cancelled`, Joby a rezervace vypořádané a autorizace zneplatněné; vítězný handoff refund odmítne a přesměruje child do shipped větve.
