...
Nový preflight a referenční slice se provedou pro každou dotčenou položku. Protože order-level minimum, balení a doprava závisí na společném výsledku, nový cenový snapshot a `ShipmentPlan` se potom přepočítají nad **celým zbývajícím batch** včetně nezměněných položek; zákazník přijme výsledný cenový rozdíl přes tokenizovaný odkaz. Stejně tak příkaz přijetí revize vždy vytvoří čerstvý `EligibilitySnapshot` a atomicky získá kompletní batch `PhaseReservationSet` pro všechny zbývající sloty, gramáž a kapacitu, přestože náhradní geometrie se týká jen `affected_order_item_ids`. Dokud to nelze, přijetí se necommitne, revize zůstává `awaiting_capacity` a batch se neaktivuje.
