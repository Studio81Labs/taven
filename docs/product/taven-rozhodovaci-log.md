...
| 71 | **`SliceResult` klíč obsahuje i `profile_kind`** | reference a machine profily mají oddělené tabulky a mohou mít stejná lokální čísla verzí; bez typu by se jejich cache klíče mohly srazit | společný neoznačený `profile_version` | zpřesňuje #62 a #70 |
