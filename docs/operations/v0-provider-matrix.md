...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runtime host                              | Existing owner-operated x86-64 Linux VPS; self-hosted Coolify manages independently created backend, web, admin, worker, database, cache, storage, proxy, and monitoring resources in a dedicated Taven team; minimum target 4 vCPU, 16 GiB RAM, 200 GB SSD                                                                                                                                                                                                                                | Coolify owner account and recovery material stay in the owner's password manager; runtime files remain under `/etc/taven/secrets`                                                                                                                                                                                                                                                                      | CZK 0 incremental while existing capacity and the self-hosted Coolify plan are sufficient; no per-request fee                                                                                      | Deploy the same independently pinned OCI resources directly on any Docker/OCI host or through another control plane; restore from the documented off-host backup                                                                |
...
- [ ] Build independent production images for backend, web, admin, and worker.
      Put each application and required worker in its own Coolify resource in a
      dedicated team and disable automatic Git deployment. Make GitHub Actions
      serialize the production workflow and, only after required checks, PATCH
      every resource's revision pin to `GITHUB_SHA` with the `read` + `write` pin
      token. Read each pin back and stop before deployment on any mismatch. Then
      call the separate deploy-only webhook, run migration once, and verify the
      completed deployment record reports the same commit and healthy resources.
      Record one release manifest containing that commit and every resulting image
      digest. Add a local integration Compose profile that builds those same
      images with local dependencies and no production credentials. Prove both
      the local stack and rollback to the prior production manifest without
      reversing a migrated schema.
