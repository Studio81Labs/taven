...
- **Related decisions:** [ADR 0006](../decisions/0006-run-v0-on-an-owner-operated-vps.md),
  ...
  [ADR 0027](../decisions/0027-require-atomic-object-creation.md)
...
**Storage correction (2026-09-24):** ADR 0027 selects the exact ADR 0026 pinned
self-hosted Silo for v0 live writes. Garage is unsupported for live writes and
remains a failing diagnostic contract target. R2 remains EU off-host backup and
an unqualified live fallback candidate. The storage rows and checklist below
describe the selected target, not a completed deployment. Issue #252 must
qualify its production security, capacity, credentials, atomic pre-start gate,
backup/restore and retained-data cutover before target activation under #39.

## Selected services

| Capability                                | Selected v0 plan and runtime                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Test or credential path                                                                                                                                                                                                                                                                                                                                                                                | Idle and usage cost                                                                                                                                                                                | Exit or fallback                                                                                                                                                                                                           |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runtime host                              | Existing owner-operated x86-64 Linux VPS; self-hosted Coolify manages the backend Compose resource plus independent web, admin, database, cache, storage, proxy, and monitoring resources in a dedicated Taven team; minimum target 4 vCPU, 16 GiB RAM, 200 GB SSD                                                                                                                                                                                                                         | Coolify owner account and recovery material stay in the owner's password manager; runtime files remain under `/etc/taven/secrets`                                                                                                                                                                                                                                                                      | CZK 0 incremental while existing capacity and the self-hosted Coolify plan are sufficient; no per-request fee                                                                                      | Deploy the same independently pinned OCI resources directly on any Docker/OCI host or through another control plane; restore from the documented off-host backup                                                           |
| Builds and image delivery                 | Coolify automatic Git deployment is disabled. GitHub Actions serializes production, verifies the workflow SHA, writes and reads back that exact revision pin for every Coolify application resource, then invokes the deploy-only webhook; Coolify builds the independently managed resources from that revision, runs migration once before activation, and records the commit plus resulting image digests in the release manifest; the prior successful revision remains rollback-ready | Workflow `GITHUB_TOKEN`; protected production variables `TAVEN_COOLIFY_API_URL` and per-resource identifiers; team-scoped `TAVEN_COOLIFY_PIN_TOKEN` with only `read` + `write`; separate `TAVEN_COOLIFY_DEPLOY_WEBHOOK` and `TAVEN_COOLIFY_DEPLOY_TOKEN` with only `deploy`                                                                                                                            | CZK 0 while this repository remains public, standard runners suffice, and Coolify remains self-hosted                                                                                              | Run the same exact-revision independent OCI resources directly over restricted SSH, on a self-hosted runner, or through another OCI-compatible control plane                                                               |
| Local development and integration testing | Fresh isolated dependency Compose stack for PostgreSQL 18, Redis 8, and pinned Silo plus a required #39 integration profile that builds and runs backend, web, admin, worker, and a pinned Garage diagnostic contract that is expected to fail immutable-write checks; the worker remains opt-in                                                                                                                                                                                           | Local-only values copied from each app's `.env.example`; no production/provider credential is required; `pnpm dev` remains the fast host-process path                                                                                                                                                                                                                                                  | CZK 0; developer machine resources only                                                                                                                                                            | Run applications on the host against the dependency stack, or replace Docker Compose with any OCI-compatible local runtime                                                                                                 |
...
| Live object storage                       | Selected: self-hosted Silo classic release `RELEASE.2026-09-16T00-00-00Z`, OCI index `sha256:635197cb9f36d01bee221d34d1c7d7960f6a95c48b0b6c01d99cd13bdae51a46`, source `2a4d51406b7ed87af5fe6fe0f801f3290f96eb3c`; private S3 API and new explicit `/srv/taven/silo` data path; #252 qualifies production runtime and application retention remains authoritative                                                                                                                          | Generic `TAVEN_S3_*` inputs; local Silo/Garage values stay in ignored `.env`; production uses separate least-privilege S3 files under `/etc/taven/secrets/backend`, `/retention-worker`, and `/slicer-worker`; #249 proves the Silo contract; #252 qualifies production identities and pre-start capability                                                                                            | CZK 0 license/incremental host cost                                                                                                                                                                | Use R2 EU or another S3 store only after the full atomic-write and retention contract qualifies; reconcile durable retention, copy/checksum only live referenced objects, and verify expired-object absence before cutover |
| Off-host backup and object-store fallback | Cloudflare R2 Standard storage with the immutable `eu` jurisdiction; Coolify PostgreSQL backups and client-side-encrypted restic Silo recovery snapshots use separate private buckets                                                                                                                                                                                                                                                                                                      | The Coolify DB bucket credential is configured only in Coolify and the owner's password manager. Separate restic `CLOUDFLARE_ACCOUNT_ID`, `TAVEN_BACKUP_R2_ACCESS_KEY_ID`, `TAVEN_BACKUP_R2_SECRET_ACCESS_KEY`, and `TAVEN_BACKUP_RESTIC_PASSWORD` values are escrowed in that password manager and the protected GitHub production Environment; runtime copies stay under `/etc/taven/secrets/backup` | First 10 GB-month, 1m Class A, and 10m Class B operations per month included; then USD 0.015/GB-month, USD 4.50/m Class A, USD 0.36/m Class B; egress free                                         | Restore to qualified Silo; use R2 live only after its full contract qualifies, or copy both backup buckets to another S3-compatible target                                                                                 |
...
| DNS, proxy, and edge TLS                  | Cloudflare Free authoritative DNS and proxy for `taven.cz`; Coolify-managed Traefik provides strict-TLS origin termination for the independently managed resources                                                                                                                                                                                                                                                                                                                         | Scoped Cloudflare DNS token in `CLOUDFLARE_API_TOKEN`; no global API key; Coolify/Traefik origin material remains in the owner-managed Coolify/host configuration                                                                                                                                                                                                                                      | CZK 0 on Free; no paid edge add-on selected                                                                                                                                                        | Export DNS records, change authoritative nameservers, and expose the same Coolify-managed origin through another DNS/CDN provider                                                                                          |
...
- [ ] Keep PostgreSQL, Redis, Silo, and monitoring ports private. Expose only
      Coolify-managed Traefik HTTPS and source-restricted SSH. Restrict the origin to Cloudflare
      address ranges where operational access permits it.
- [ ] Build independently deployable production images for backend, web, admin,
      and worker. Deploy the backend API and all backend-image workers together
      from `infra/coolify/docker-compose.yml` as one Coolify backend resource;
      keep web, admin, slicer, and Orca as their separately managed resources.
      Disable automatic Git deployment. Make GitHub Actions serialize the
      production workflow and, only after required checks, PATCH each resource's
      revision pin to `GITHUB_SHA` with the `read` + `write` pin token. Read each
      pin back and stop before deployment on any mismatch. Then call the separate
      deploy-only webhook, run migration once, and verify the completed
      deployment record reports the same commit and healthy resources. Record
      one release manifest containing that commit and every resulting image
      digest. The local Compose stack builds the same application images with
      local dependencies and no production credentials. Prove both the local
      stack and rollback to the prior production manifest without reversing a
      migrated schema.
...
- [ ] Create separate least-privilege database and S3 credentials for the
      ...
      operations differ. Do not use Silo root credentials in an app
      container.
- [ ] Put PostgreSQL data, Redis AOF, and Silo data on explicit persistent
...
- [ ] Create private live, quarantine, and backup scopes or prefixes. Apply
      ...
      age alone.
- [ ] Upgrade Silo only by explicit release, source commit, and OCI digest.
      Before promotion, run the full Silo S3 compatibility suite; Garage remains
      an unsupported failing diagnostic. Prove signed upload/download with a
      signed checksum header, HEAD checksum, range read, atomic conditional
      creation, copy, list, retention deletion/HEAD, encrypted backup, and
      restore. Retain the prior digest and a compatible backup until rollback
      is no longer needed.
...
- [ ] Make the nightly full recovery point a single write-free maintenance
      ...
      its verified R2 artifact. Then stop Silo and create one read-only
      filesystem snapshot that atomically covers the complete Silo data and
      required configuration/identity state. Record an immutable pending
      manifest that pairs the Coolify
      execution ID and artifact checksum with the frozen Silo snapshot ID,
      then restart Silo and reopen writes. Encrypted restic reads only that
      ...
      an atomic LVM, Btrfs, or ZFS snapshot, keep Silo quiesced until a complete
      ...
      documented.
- [ ] Keep every not-yet-expired live object in Silo for its full application
...
- [ ] Treat v0 backup objectives as RPO 24 hours and best-effort RTO 4 hours.
      ...
      the paired Coolify PostgreSQL artifact plus complete Silo data/configuration
      snapshot
      ...
      Verify restored Silo data, metadata, identities, policies and referenced
      checksums before traffic resumes; #252 owns the rehearsal.
...
- [ ] Add the domain to Cloudflare Free, delegate nameservers, enable DNSSEC, and
      ...
      production-ready.
- [ ] Use strict TLS from Cloudflare to Coolify-managed Traefik, redirect HTTP, configure CORS and
...
- [ ] Alert on public health, host/disk pressure, PostgreSQL/Redis/Silo health,
...
- [Issue #39](https://github.com/Studio81Labs/taven/issues/39) adds the real
  ...
  provider-neutral S3 presigner adjustment and Silo/Garage contract matrix,
