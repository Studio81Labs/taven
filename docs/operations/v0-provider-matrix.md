...
- [ ] Run a nightly consistent PostgreSQL dump. For Garage, enter storage-write
      maintenance, drain in-flight uploads/slices/retention deletions, stop
      Garage, and create one read-only filesystem snapshot that atomically
      covers both its metadata and block-data paths. Restart Garage before
      encrypted restic reads that snapshot into R2. If the selected host cannot
      provide an atomic LVM, Btrfs, or ZFS snapshot, keep Garage quiesced until a
      complete staged copy has finished; never traverse its live writable
      volume. Keep seven daily and four weekly snapshots, prune daily, and cap
      the oldest recoverable customer-content copy at 35 days unless a legal
      hold is documented.
...
- [ ] Treat v0 backup objectives as RPO 24 hours and best-effort RTO 4 hours.
      ...
      the paired PostgreSQL dump plus Garage metadata/blocks snapshot at least
      quarterly and before a major database or storage change. Run Garage table
      and block repair/verification after restore before traffic resumes.
