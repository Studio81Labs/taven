...
## Origin-ingress amendment

[ADR 0023](../decisions/0023-use-coolify-traefik-for-origin-ingress.md) / #164
replaces Caddy with Coolify-managed Traefik for staging and production. Use one
origin proxy, Taven-scoped DNS-01 certificate handling, an actual-peer Cloudflare
source gate and a distinct verified-client staging/admin allowlist. Preserve
shared-host services and all other provider selections. #39/PR #162 owns the
configuration/runbook/tests; live activation requires the stated technical checks.
Local/private development continues with v0.1 legal drafts while remote ingress
is prepared. Final legal review gates only public production activation.
...
| DNS, proxy, and edge TLS                  | Cloudflare Free authoritative DNS and proxy for `taven.cz`; Coolify-managed Traefik provides strict-TLS origin termination (ADR 0023)                                                                                                                                                                                                                                                                  | Scoped Cloudflare DNS credential supplied only to the proxy for the Taven DNS-01 resolver; no global API key; certificate/ACME state in protected persistent proxy storage (ADR 0023)                                                                                                                                                                                                                  | CZK 0 on Free; no paid edge add-on selected                                                                                                                                                        | Export DNS records, change authoritative nameservers, and expose the same Traefik origin through another DNS/CDN provider                                                                                                       |
...
- [ ] Keep PostgreSQL, Redis, Garage, and monitoring ports private. Expose only
      Traefik HTTPS and source-restricted SSH. Restrict the origin to Cloudflare
...
- [ ] Add the domain to Cloudflare Free, delegate nameservers, enable DNSSEC, and
      ...
      production-ready.
- [ ] Use Full (strict) TLS from Cloudflare to Traefik, redirect HTTP, configure CORS and
