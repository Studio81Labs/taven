...
| Isolated real API     | Web CI `web: isolated real API browser checkout`                                                                                                                                                                           | Production-built Nuxt and Nest processes with PostgreSQL 18, Redis, MinIO, fixture slicing and sandbox payment. Test-only legal revisions are published into that isolated database. Browser actions and database effects are both asserted. | Staging deployment, real Orca, Comgate, carrier, email or manual fulfilment.                                                                  |
