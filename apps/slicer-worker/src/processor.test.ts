...
class MemoryStore implements WorkerObjectStore {
  ...
  readonly readLimits: { key: string; maximum: number }[] = [];

  async read(key: string, maximum: number, expected?: string) {
    this.readLimits.push({ key, maximum });
    ...
    if (existing) {
      ...
      if (
        ...
        (existingMetadata?.immutableInputFingerprintSha256 !==
          ...
          existingMetadata.metadataContentSha256 !== existingSha256)
      ) {
        throw new SlicingWorkerError(
          "deterministic_invalid",
          "INVALID_MODEL",
      ...
      return { sha256: existingSha256, cacheHit: true };
    }
...
class RacingWriteStore extends MemoryStore {
  constructor(private readonly winner: Uint8Array) {
    super();
  }
  ...
  override async write(
    ...
    if (!this.objects.has(key)) {
      this.objects.set(key, new Uint8Array(this.winner));
      if (immutableInputFingerprintSha256 !== undefined) {
        this.metadata.set(key, {
          immutableInputFingerprintSha256,
          metadataContentSha256: sha256(this.winner),
        });
      }
    }
    return super.write(
...
class FakeEngine implements OrcaEngine {
  ...
  constructor(
    private readonly thinWallFeatureCount = 1,
    private readonly generatedAt = "2026-08-31 at 12:00:00",
  ) {}

  async slice(input: OrcaSliceRequest) {
    this.calls.push(input);
    this.profileContents.push(
      await Promise.all(
        input.profilePaths.map((profile) => readFile(profile, "utf8")),
      ),
    );
    const gcode = `; generated by OrcaSlicer 2.4.2 on ${this.generatedAt}\n; total estimated time: ${input.copies}m 0s\n; filament used [g] = ${input.copies}\n; EXECUTABLE_BLOCK_START\nM83\n; CHANGE_LAYER\n; FEATURE: Outer wall\nG1 X1 E2\n; FEATURE: Support\nG1 X2 E1\n; FEATURE: Gap infill\nG1 X3 E1\nM104 S0\n; EXECUTABLE_BLOCK_END\n`;
...
const presetBundle = (...presets: Record<string, unknown>[]) =>
  new TextEncoder().encode(JSON.stringify({ bundleVersion: 1, presets }));
const machineBundle = () =>
  presetBundle(
...
describe("SlicingProcessor", () => {
  ...
  it("loads an initial UTF-16 reference-profile snapshot and reuses its dispatch artifact", async () => {
    const store = new MemoryStore();
    const engine = new FakeEngine();
    ...
    const referenceProfile = presetBundle(
      { type: "machine", name: "machine" },
      { type: "process", é: "decomposed", name: "process", é: "composed" },
      { type: "filament", name: "filament" },
    );
    const printConfig = presetBundle({ name: "print-config" });
    ...
    secondStore.objects.set(`geometries/${ids.geometry}/canonical`, geometry);
    secondStore.objects.set(
      `slicer-revisions/${referenceHash}/settings.json`,
      referenceProfile,
    );
    secondStore.objects.set(
    ...
    const independentlySliced = await new SlicingProcessor(
      secondStore,
      new FakeEngine(1, "2031-01-01 at 00:00:00"),
      config,
    ).process(job);
    expect(independentlySliced.outcome).toMatchObject({
      status: "succeeded",
      artifact: { sha256: first.outcome.artifact.sha256 },
    });
    expect(
      new TextDecoder().decode(
        secondStore.objects.get(
          referenceArtifactObjectKey(job.inputFingerprintSha256),
        )!,
      ),
    ).toMatch(
      /^; generated by OrcaSlicer 2\.4\.2 on 2026-07-06 at 14:18:06$/mu,
    );
  });
  ...
  it("reuses immutable candidate occupancy caches and never persists G-code", async () => {
    ...
    const revisionContents = [
      machineBundle(),
      presetBundle({ name: "calibration" }),
      presetBundle({ name: "print-config" }),
    ];
    ...
    const crossEffectJob = envelope("candidate_estimate", {
      ...base,
      shipmentPlanId: ids.job,
      occupancySliceTargets: targets,
    }) as SlicingJob;
    const crossEffect = await processor.process(crossEffectJob);
    expect(first).toEqual(second);
    expect(first.outcome.status).toBe("succeeded");
    expect(crossEffect.outcome).toMatchObject({ status: "succeeded" });
    ...
    expect(
      store.readLimits
        .filter(({ key }) => key.startsWith("slice-metrics/"))
        .every(({ maximum }) => maximum === 64 * 1024),
    ).toBe(true);
    for (const target of targets) {
      expect(store.metadata.get(target.analysisObjectKey)).toMatchObject({
        immutableInputFingerprintSha256: target.cacheIdentitySha256,
      });
    }
    expect(
  ...
  it("does not accept a different immutable artifact after a write race", async () => {
    ...
    const geometryHash = sha256(geometry);
    store.objects.set(`geometries/${ids.geometry}/canonical`, geometry);
    const referenceProfile = machineBundle();
    const printConfig = presetBundle({ name: "config" });
    const referenceHash = sha256(referenceProfile);
    const configHash = sha256(printConfig);
    store.objects.set(
      `slicer-revisions/${referenceHash}/settings.json`,
  ...
  it("adopts a self-consistent winner after a concurrent immutable write", async () => {
    const winner = new TextEncoder().encode("winner artifact");
    const store = new RacingWriteStore(winner);
    const processor = new SlicingProcessor(
      store,
      new FakeEngine(),
      config,
    ) as unknown as {
    ...
    const objectKey = `reference-slices/${fingerprint}/metrics.gcode`;

    const artifact = await processor.reusableArtifact(
      objectKey,
      "text/x.gcode",
      async () => new TextEncoder().encode("losing artifact"),
      fingerprint,
    );

    expect(artifact).toMatchObject({ bytes: winner, sha256: sha256(winner) });
    expect(store.objects.get(objectKey)).toEqual(winner);
    expect(store.metadata.get(objectKey)).toEqual({
      immutableInputFingerprintSha256: fingerprint,
      metadataContentSha256: sha256(winner),
    });
  });
});
