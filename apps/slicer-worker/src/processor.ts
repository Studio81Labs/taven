...
const normalizedReferenceTimestamp = new Date(
  runtimeLock.sourceDateEpoch * 1_000,
)
  .toISOString()
  .replace("T", " at ")
  .replace(".000Z", "");

export function normalizeReferenceArtifact(bytes: Uint8Array): Uint8Array {
  const source = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  return new TextEncoder().encode(
    source.replace(
      /^(; generated by OrcaSlicer \S+ on )\d{4}-\d{2}-\d{2} at \d{2}:\d{2}:\d{2}$/mu,
      `$1${normalizedReferenceTimestamp}`,
    ),
  );
}
...
export class SlicingProcessor {
  ...
  private async reusableArtifact(
    ...
    maximumBytes = this.config.limits.artifactBytes,
  ): Promise<{ bytes: Uint8Array; sha256: string }> {
    const cached = await this.store.read(objectKey, maximumBytes);
    if (cached) {
    ...
    try {
      ...
      if (!winner) {
        throw new SlicingWorkerError(
          ...
          "Immutable artifact disappeared after storage write",
          5_000,
        );
      }
      ...
      const winner = await this.store.read(objectKey, maximumBytes);
      if (winner) {
        this.assertReusableIdentity(winner, immutableInputFingerprintSha256);
        validate?.(winner);
        return winner;
  ...
  private async machineOccupancy(
    ...
    const artifact = await this.reusableArtifact(
      ...
      target.cacheIdentitySha256,
      (stored) => this.validateOccupancyArtifact(stored, target),
      64 * 1024,
    );
    return this.validateOccupancyArtifact(artifact, target);
  }

  private validateOccupancyArtifact(
    artifact: { bytes: Uint8Array; sha256: string },
    target: {
