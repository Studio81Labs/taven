...
describe("runFixtureSlicingJob", () => {
  it("discovers the checked-in two-object 3MF without changing other sources", () => {
    ...
    const twoBodyJob = fixtureJob("model_inspection", twoBodyInput);
    ...
    const inspectSelection = (bodyIds: string[]) =>
      runFixtureSlicingJob(
        fixtureJob("model_inspection", {
          ...twoBodyInput,
          operation: {
            mode: "canonicalize_selection",
            sourceInspectionFingerprintSha256:
              twoBodyJob.inputFingerprintSha256,
            bodyIds,
    ...
    expect(inspectSelection(["body-0002"])).toMatchObject({
      outcome: {
        status: "succeeded",
        metrics: {
          bodyCount: 1,
          objectCount: 1,
          boundingBox: { xMicrometers: "20000" },
        },
        bodies: [{ bodyId: "body-0002" }],
      },
    });
    expect(inspectSelection(["body-0001", "body-0002"])).toMatchObject({
      outcome: {
        status: "succeeded",
        metrics: {
          bodyCount: 2,
