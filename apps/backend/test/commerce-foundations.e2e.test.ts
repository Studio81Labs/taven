...
describe("commerce persistence foundations", () => {
  ...
  it("requires descriptions for custom-service quote and order items", async () => {
    await rollback("custom-service-description", async (client, fixtures) => {
      const foundation = await fixtures.createFoundation(
      ...
      const quoteRequestId = fixtures.id("custom-service-request");
      const quoteId = fixtures.id("custom-service-quote");
      const orderId = fixtures.id("custom-service-order");
      const now = new Date();
      await client.query(
      ...
      await client.query(
        ...
        [
          orderId,
          foundation.customerId,
          `CUSTOM-SERVICE-${orderId.slice(0, 8)}`,
          now,
        ],
      );
      ...
      await expectQueryError(
        client,
        "null_quote_service_description",
        () =>
          client.query(
            `INSERT INTO quote_items
               (id, quote_id, ordinal, kind, service_description, quantity, created_at)
             VALUES ($1,$2,0,'CUSTOM_SERVICE',NULL,1,$3)`,
            [fixtures.id("invalid-custom-quote-item"), quoteId, now],
          ),
        { code: "23514", constraint: "quote_items_kind_shape_check" },
      );
      await expectQueryError(
        client,
        "null_order_service_description",
        () =>
          client.query(
            `INSERT INTO order_items
               (id, order_id, ordinal, kind, service_description, quantity, created_at)
             VALUES ($1,$2,99,'CUSTOM_SERVICE',NULL,1,$3)`,
            [fixtures.id("invalid-custom-order-item"), orderId, now],
          ),
        { code: "23514", constraint: "order_items_kind_shape_check" },
      );
    });
  });
