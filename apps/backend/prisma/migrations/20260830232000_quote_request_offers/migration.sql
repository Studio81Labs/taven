-- Persist the complete individual-request/offer contract and admit explicit
-- custom-service lines without weakening the existing model-backed item path.
...
ALTER TABLE "quote_items"
    ADD COLUMN "kind" "commerce_item_kind" NOT NULL DEFAULT 'MODEL',
    ADD COLUMN "service_description" TEXT,
    ALTER COLUMN "source_model_file_id" DROP NOT NULL,
    ALTER COLUMN "model_geometry_id" DROP NOT NULL,
    ALTER COLUMN "print_config_revision_id" DROP NOT NULL,
    ALTER COLUMN "material" DROP NOT NULL,
    DROP CONSTRAINT "quote_items_ordinal_quantity_check",
    ADD CONSTRAINT "quote_items_kind_shape_check" CHECK (
        "ordinal" >= 0 AND "quantity" > 0 AND (
            (
                "kind" = 'MODEL'
                AND "service_description" IS NULL
                AND "source_model_file_id" IS NOT NULL
                AND "model_geometry_id" IS NOT NULL
                AND "print_config_revision_id" IS NOT NULL
                ...
                AND "print_config_revision_id" IS NULL
                AND "reference_slice_result_id" IS NULL
                AND "tail_reference_slice_result_id" IS NULL
                AND "reference_parts_per_plate" IS NULL
...
ALTER TABLE "order_items"
    ...
    ALTER COLUMN "print_config_revision_id" DROP NOT NULL,
    ALTER COLUMN "material" DROP NOT NULL,
    DROP CONSTRAINT "order_items_ordinal_quantity_check",
...
DROP TRIGGER "quote_items_geometry_source_available" ON "quote_items";
CREATE TRIGGER "quote_items_geometry_source_available"
BEFORE INSERT ON "quote_items"
FOR EACH ROW
WHEN (NEW."kind" = 'MODEL')
EXECUTE FUNCTION taven_assert_geometry_usage_available();

DROP TRIGGER "order_items_geometry_source_available" ON "order_items";
...
BEGIN
    IF NOT EXISTS (
        ...
        JOIN "quote_items" quote_item ON quote_item."id" = NEW."quote_item_id"
        ...
        WHERE order_item."id" = NEW."order_item_id"
          ...
          AND order_item."kind" = quote_item."kind"
          AND order_item."service_description" IS NOT DISTINCT FROM quote_item."service_description"
          AND order_item."source_model_file_id" IS NOT DISTINCT FROM quote_item."source_model_file_id"
          AND order_item."model_geometry_id" IS NOT DISTINCT FROM quote_item."model_geometry_id"
          AND order_item."print_config_revision_id" IS NOT DISTINCT FROM quote_item."print_config_revision_id"
          ...
          AND order_item."material" IS NOT DISTINCT FROM quote_item."material"
        ...
        RAISE EXCEPTION 'individual order item must exactly copy an item from its accepted quote'
            USING ERRCODE = '23514',
                  CONSTRAINT = 'individual_order_item_source_match_check';
    END IF;
